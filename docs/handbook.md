...
**Someone found a tool, library or package — will it make a difference here?** — `/tool-compare` (verdict + evaluation doc; upstream issues and a competitor comparison only if you ask)
...
| `tool-compare` | Should this project adopt an external tool, library, package or service? Checks `.claude/project-docs/` for a standing verdict first, pins the version, reads **source not README**, and builds a claim-vs-measured table, including what the tool's own tests fake (a green suite that fakes HTTP proves nothing about URLs). Establishes whether the project *has* the problem, and whether the evidence source could even see it. Prices the cost, names the project-specific risk the authors couldn't see, and writes `<tool>-evaluation.md` whether adopted or rejected. **Optional, only on request:** file upstream issues (pinned permalinks, dependency-ordered cross-refs) and a competitor comparison and roadmap with every cited fact re-verified. Verdict: adopt / adopt part / not now / skip. Restraint-gated: "no demonstrated need" is complete. *(custom, local)* |
