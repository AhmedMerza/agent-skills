...
- **MR suggestion** uses `glab mr create` (auto-detects remote/project) or the global `/skill:mr-create`.
- **`commit-agent` is optional** — if a `commit-agent` subagent is defined (project or user
  scope), spawn it via the Agent tool for 2–50 file changes for context isolation; otherwise run
  the analysis inline. Don't fail if it's absent.
