...
handover, does not commit. It produces a read and a recommendation; the user acts on it. You get
no write or edit tools here for exactly this reason — if you find yourself wanting them, you have
misread the job.
...
- Constraints or gotchas discovered this session that are not in the code, AGENTS.md, or memory
...
list of what `/skill:handover-save` would need to capture if the user does clear.
...
- **HANDOVER + CLEAR** — the session is at a task boundary, **work continues past it**, and the
  ...
  should go in the handover (the LIVE list), then let the user run `/skill:handover-save` and
  start a new session themselves.
- **CLEAR** — the task is *finished*: merged, filed, shipped, nothing in flight. LIVE is empty,
  not merely small. Recommend starting a new session **alone — no handover.** A handover doc
  carries unfinished
...
number is worse than no number — the client's status line already shows the real one, and it is
what made the user invoke this. Open with what the session is *for*. If the user quotes a figure,
use theirs.
