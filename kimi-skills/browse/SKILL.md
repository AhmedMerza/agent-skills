...
(base URL, login flow, default user) come from an optional `<repo>/.kimi-code/browse-config.json`;
credentials are **never** stored — they come from `PW_EMAIL` / `PW_PASS` env at run time.

- **Config**: `.kimi-code/browse-config.json`. Keys: `baseUrl`, `loginPath`, `defaultUser`, `channel`, `playwrightDir`, `outDir`,
...
1. **Ensure credentials.** Login needs `PW_PASS` (and an email via `--user`, `PW_EMAIL`, or
   ...
   ```bash
   PW_PASS='<password>' node ${KIMI_SKILL_DIR}/browse.mjs $ARGUMENTS
   ```
   Use a timeout of 120000ms. Run from the **project root** (so the helper finds
   `.kimi-code/browse-config.json` and the project's Playwright).
...
## Notes
- Base URL, login behavior, and the default user are per-project (`.kimi-code/browse-config.json`).
