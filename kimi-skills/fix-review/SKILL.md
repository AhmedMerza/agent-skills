...
- **Override wins:** `.kimi-code/repo-config.json` with `"provider": "github"` or `"gitlab"`.
...
1. **Where the findings come from:**
   - A `/skill:mr-review` report (or ad-hoc review) **in this conversation** → use it.
...
2. **Post the defects that have no thread yet** (GitLab). Write them with `json.dump` to
   `.kimi-code/tmp/mr-review/<N>/findings.json` —
   ...
   Findings `/skill:mr-review --comment` (or an earlier run) already posted come back `exists <id>` —
   nothing is duplicated. Keep the finding → `discussion_id` map for Phase 3. Report any `FAILED` line.
   GitHub: follow `/skill:mr-review` Step 7's GitHub path.
