...
3. **Optional extras** — if `.kimi-code/gitlab-config.json` exists in the current repo, load `developers` (assignee/reviewer shortcuts) and `labels` (auto-label rules) from it. If it is **absent**, those features degrade gracefully: ask the user for the assignee/reviewer/labels instead of resolving a shortcut.
...
1. **Issue type mapping** (from `.kimi-code/gitlab-config.json` → `labels.commitTypes`):
   - Bug → `bug`
   - Feature → `feature`
   - Enhancement → `enhancement`

2. **File pattern matching** (from `.kimi-code/gitlab-config.json` → `labels.filePatterns`):
...
**IMPORTANT: Always use `glab api` directly via Bash tool. Do NOT use the Agent/subagent tool for API calls.**
...
## Options
- `--assign <developer>` - Assign to developer (use a shortcut from `.kimi-code/gitlab-config.json`, e.g. `alice`, `bob`, `carol`, or pass a raw GitLab username)
...
**IMPORTANT: Always use `glab api` directly via Bash tool for issue creation. Do NOT use the Agent/subagent tool.**
...
## Related Commands
- `/skill:mr-create` - Create merge request (after fixing the issue)
- `/mr-assign` - Assign MR to developer
- `/plan` - Plan implementation for a complex issue
- `/skill:commit` - Commit the fix with semantic message
...
- The skill respects `.kimi-code/gitlab-config.json` for developer shortcuts and label mappings
- All issue creation requires explicit user permission (shown as preview first)

---
*Created: 2026-02-18 | Config: .kimi-code/gitlab-config.json, .gitlab/issue_templates/*
