---
name: handover-save
description: Materialize the current conversation's plan into a durable handover doc under .kimi-code/handover/ (survives starting a new session and session handoffs).
type: flow
---

You are capturing the plan we have developed **in this conversation** into a durable, gitignored
handover document, so it survives starting a new session, context compaction, and handoff to a
fresh session or another engineer. This is NOT a fresh plan — extract what we already worked out.
...
- Slug: use `$ARGUMENTS` if provided. Otherwise run `git branch --show-current` and derive a slug
  ...
  (detached HEAD or not a git repo), ask the user for a slug — never write to `.kimi-code/handover/.md`.
- **Sanity-check a branch-derived slug against what this conversation is actually about.** If the
  ...
  before writing — otherwise the plan is filed under an unrelated name and `/skill:handover-resume` on that
  branch will never surface it.
- Path: `.kimi-code/handover/<slug>.md`.
...
## 3. Write the doc in this structure (mirror .kimi-code/handover/mr-2b-auto-exclude-reconcile.md)
...
(personal scratch) and can be reloaded later with `/skill:handover-resume <slug>`.
