...
3. **Resolve developers** (if provided): Look up usernames (and ids, for GitLab) from shortcuts in `.kimi-code/repo-config.json`
...
- `--guide` - Generate a reviewer's guide (see `/skill:mr-guide`) from the branch diff and open the MR with it in the description
...
- Maps patterns to code owners (from `.kimi-code/code-ownership.json`)
...
**Code Ownership Rules** (example `.kimi-code/code-ownership.json` — genericize to the repo's real layout):
...
**Template Locations**:
- `.kimi-code/mr-templates/feature.md`
- `.kimi-code/mr-templates/bugfix.md`
- `.kimi-code/mr-templates/hotfix.md`
- `.kimi-code/mr-templates/refactor.md`
- `.kimi-code/mr-templates/documentation.md`
...
Use the resolved provider's CLI. **Run the CLI directly via the Bash tool for creation. Do NOT use the Agent/subagent tool.**
