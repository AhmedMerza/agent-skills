...
Use `/skill:mr-review` (findings), `/nitpick` (strict), or `/skill:ship-check` (pre-merge gate) for that.
...
1. **Detect the provider** from `git remote get-url origin` — host `github.com` → **GitHub** (`gh`, "PR");
   ...
   `.kimi-code/repo-config.json` → `"provider"` overrides.
...
3. **No argument, no MR yet** → the **branch diff** against its target branch
   ...
   This is the path `/skill:mr-create --guide` uses.
...
3. Capture **after** on the current branch, then **before** on the target branch, via `/skill:browse`.
...
- **No MR exists** → there is nothing to publish. Print the guide and say
  `No MR yet — run /skill:mr-create --guide to open one with this guide in the description.`
...
3. **Delegate the analysis.** For a diff over ~10 files, spawn parallel subagents via the Agent
   tool — one for backend/data-model, one for frontend/UI-states, one for
...
- `/skill:mr-create --guide` — generate the guide from the branch diff and open the MR with it in
  the description.
- `/skill:mr-review` — if the MR description contains a `<!-- mr-guide -->` block, read it for
